...
watch(
    [() => props.show, () => props.dateFrom, () => props.dateTo],
    ([show]) => {
        if (show) {
            loadOwnTasks();
...
function addContinuationMarkers(doc) {
    ...
    for (let i = 1; i < totalPages; i++) {
        ...
        doc.text("Continued on next page...", pageWidth / 2, pageHeight - 16, {
            align: "center",
        });
    }
}
...
async function exportIndividualPdfFallback() {
    ...
    doc.setFontSize(11);
    doc.text(
        "Caraga State University - Engineering and Construction Office",
        pageWidth / 2,
        46,
        { align: "center" },
    );
    ...
    const head = [
        ["No.", "Date", "Task/Subtask", "Description", "Remarks", "Drive Link"],
    ];
    ...
    doc.text(
        `System-generated on ${generated}. Do not alter.`,
        margin,
        pageHeight - 14,
    );
...
async function exportPdf() {
    ...
    const exportHiddenNodes = root.querySelectorAll(
    ...
    try {
        ...
        while (heightLeft > 0) {
            ...
            const sourceSliceHeight = Math.round(
                (sliceHeight / scaledImgHeight) * canvas.height,
            );
        ...
        console.warn(
            "[IndividualAccomplishmentReport] PDF capture fallback:",
            error,
        );
...
const loadUnitHead = async () => {
    ...
    try {
        const response = await apiFetch(
            `/report/load_unit_head?unitId=${unitId}`,
            { method: "GET" },
        );
        const result = await response.json();
        ...
        if (response.ok) {
            unitHeadInfo.value = {
                name: result.data,
                title: userUnitName.value
                    ? `Unit Head, ${userUnitName.value}`
                    : "Unit Head",
            };
        } else throw new Error(result?.error);
    } catch (err) {
...
const approvedTasksInPeriod = computed(() => {
    ...
    return ownTasks.value.filter(
        (t) => inPeriod(t.startDate) || inPeriod(t.endDate),
    );
});
...
const reportRows = computed(() => {
    ...
    const fmt = (s) => {
        const d = new Date(s);
        return isNaN(d) ? "" : d.toLocaleDateString("en-PH", FMT);
    };
    ...
    approvedTasksInPeriod.value.forEach((t) => {
        const activities =
            t.subtaskNames && t.subtaskNames.length ? t.subtaskNames : [""];
    ...
    console.log(approvedTasksInPeriod.value);
    return rows;
});
...
<template>
    ...
    >
        ...
        >
            ...
            <!-- ── Header ── -->
            <div
                class="relative flex-shrink-0 border-b border-gray-100 px-4 pb-4 pt-5 sm:px-8 sm:pb-5 sm:pt-6"
            >
                <img
                    src="/images/csu.png"
                    alt=""
                ...
                <div class="flex flex-col items-center">
                    ...
                    />
                    <p
                        class="text-[10px] font-bold tracking-[0.15em] text-green-700 uppercase"
                    >
                        Caraga State University
                    </p>
                    <h1
                        class="text-center text-lg font-bold text-gray-900 sm:text-xl"
                    >
                        ACCOMPLISHMENT REPORT
                    </h1>
                    <p
                        class="mt-0.5 text-center text-[12px] tracking-wide text-gray-500 sm:text-[13px]"
                    >
                        Engineering and Construction Office
                    </p>
                    <p class="text-[11px] text-green-800 font-semibold mt-1">
                        {{ periodLabel }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-auto px-4 py-5 sm:px-8 sm:py-6">
                <p
                    v-if="emptyMessage"
                    class="mb-4 rounded-lg bg-amber-50 px-3 py-2 text-[11px] text-amber-800"
                >
                    {{ emptyMessage }}
                </p>
                <table class="w-full border-collapse text-[10px] table-fixed">
                    <thead>
                        <tr
                            class="bg-green-800 text-white text-[9px] uppercase tracking-normal"
                        >
                            <th
                                class="border border-green-700 px-1 py-1.5 font-semibold text-center whitespace-nowrap"
                                style="width: 5%"
                            >
                                No.
                            </th>
                    ...
                    <tbody>
                        <tr v-for="(row, i) in reportRows" :key="i" class="h-8">
                            <td
                                class="border border-gray-300 px-2 py-1 text-center text-gray-600"
                            >
                                {{ row.no }}
                            </td>
                            <td
                                class="border border-gray-300 px-2 py-1 text-gray-600 whitespace-nowrap"
                            >
                                {{ row.date }}
                            </td>
                            <td
                                class="border border-gray-300 px-2 py-1 text-gray-600 break-words"
                            >
                                {{ row.task }}
                            </td>
                            <td
                            ...
                            >
                                ...
                                <span v-else class="text-[9px] text-gray-300"
                                    >—</span
                                >
                            </td>
                        </tr>
                ...
                <!-- Footer -->
                <div
                    class="mt-8 grid grid-cols-1 gap-6 text-xs text-gray-600 sm:grid-cols-3 sm:items-start sm:gap-4"
                >
                    <!-- Prepared by -->
                    <div class="sm:min-w-[220px] sm:max-w-[260px]">
                        <p
                            class="mb-6 text-gray-400 font-semibold uppercase tracking-wide text-[10px]"
                        >
                            Prepared by:
                        </p>
                        <div class="border-t border-gray-400 pt-1">
                            <p
                                class="font-bold text-gray-800 uppercase text-[11px]"
                            >
                                {{ reportName }}
                            </p>
                    ...
                    <!-- Approved by -->
                    <div
                        class="sm:min-w-[220px] sm:max-w-[260px] sm:justify-self-end sm:text-left"
                    >
                        <p
                            class="mb-6 text-gray-400 font-semibold uppercase tracking-wide text-[10px]"
                        >
                        ...
                        <div class="border-t border-gray-400 pt-1">
                            <p
                                class="font-bold text-gray-800 uppercase text-[11px]"
                            >
                                AR. Magichael B. Cloribel
                            </p>
                            <p class="text-gray-500">
                                Director, Engineering & Construction Office
                            </p>
                        </div>
                    </div>
                </div>
                <p
                    class="text-[9px] text-gray-300 italic mt-4 select-none pointer-events-none"
                >
                    System-generated on
                    {{

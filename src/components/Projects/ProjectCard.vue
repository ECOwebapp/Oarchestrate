...
const daysLeft = computed(() => {
  ...
  const target = new Date(props.project?.deadline); target.setHours(0, 0, 0, 0)
...
<template>
  <div @click="handleClick" class="relative flex flex-col rounded-2xl py-3 px-3 sm:px-4 overflow-hidden bg-white shadow-lg
    ...
    <div v-if="selectable" @click.stop="handleCheckboxClick" class="absolute top-2.5 right-2.5 z-20 w-5 h-5 rounded-md border-2 flex items-center justify-center
              transition-all duration-150 shadow-sm"
      :class="selected ? 'bg-green-700 border-green-700' : 'bg-white/90 border-gray-400 hover:border-green-600'">
    ...
    <p class="line-clamp-2 text-xs text-gray-500 italic mb-2 leading-relaxed">{{ project.description }}</p>

    <div class="flex items-center justify-between gap-2 mb-2">
      <p class="text-[10px] sm:text-xs text-gray-400 truncate flex items-center gap-1">
    ...
    <div class="flex items-center gap-2 mt-auto pt-1">
      <div class="flex-1 bg-gray-200 rounded-full h-1.5 overflow-hidden min-w-0">
        <div class="h-full rounded-full transition-all duration-500"
          :class="project?.is_completed ? 'bg-green-600' : 'bg-green-950'"
          :style="{ width: progress + '%' }" />
      </div>
      <p class="text-[10px] font-bold italic flex-shrink-0"
        :class="daysLeft < 0 && !project.director ? 'text-red-600' : project.director ? 'text-green-600' : 'text-gray-500'">
        {{ project?.is_completed ? 'Done' : daysLeft < 0 ? `${Math.abs(daysLeft)}d overdue` : daysLeft === 0 ? 'Due today!' :
          `${daysLeft}d left` }} </p>
    </div>
  </div>
</template>
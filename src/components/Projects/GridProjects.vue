...
<template>
  <div>
    <div v-if="props.projects.length < 1" class="flex flex-col items-center justify-center h-full py-20 text-gray-400">
    ...
    <div v-else class="mask-y-from-95% mask-y-to-97% h-full overflow-y-auto grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4
      ...
      <ProjectCard v-for="(project, index) in props.projects" :key="project.id" :project="project" :selectable="props.selectable"
        :selected="props.selectedIds.has(project.id)" :is-deletable="props.isDeletable(project)" @open="handleOpen(project)"

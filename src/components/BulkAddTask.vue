    ...
    <div v-if="done" class="flex-1 flex flex-col items-center justify-center gap-4 px-7 py-12">
      ...
      <div class="flex gap-3 mt-2">
        <button @click="clearAll"
          class="px-5 py-2 rounded-xl border-2 border-gray-300 text-gray-600 text-sm font-semibold
                 hover:border-green-800 hover:text-green-800 transition-colors">
          Add more
        </button>
        <button @click="emit('close')"
          class="px-5 py-2 rounded-xl bg-green-950 text-white text-sm font-semibold
    ...
    <template v-else>
      ...
      <div class="overflow-y-auto flex-1 px-7 py-5 space-y-4">
        ...
        <div class="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit">
          <button
            v-for="tab in [{ id:'paste', label:'Paste text' }, { id:'excel', label:'Upload Excel / CSV' }]"
            :key="tab.id"
        ...
        <div v-if="activeTab === 'paste'" class="space-y-2">
          <textarea
            v-model="rawInput"
            rows="7"
            placeholder="Paste your task data here (tab-separated columns):

Category / Parent Task Name (no dates = treated as header)
Task Name	TOOU	01/01/2026	28/02/2026	Green
Sub-task Name	PDU	01/02/2026	15/02/2026	Blue"
            class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 text-sm font-mono resize-y
          ...
          <button @click="parseText"
            class="w-full h-10 rounded-xl bg-green-950 text-white text-sm font-semibold
                   hover:bg-green-800 transition-colors">
            Parse &amp; Preview
          </button>
        </div>
            ...
            <div
              ...
              <div
                ...
                <div class="w-2.5 h-2.5 rounded-full flex-shrink-0"
                  :style="{ background: dotColor(task.color) }"></div>
                ...
                <div class="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  <span v-if="!task.end"
                    class="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">No deadline</span>
              ...
              <div v-if="expanded[ti]" class="border-t border-gray-100 px-4 py-4 space-y-3 bg-gray-50/60">
                ...
                <div class="flex gap-3">
                  <div class="flex-1 space-y-1">
                    ...
                    <input v-model="task.name" type="text" maxlength="100" placeholder="Task title…"
                      class="w-full border border-gray-300 rounded-lg h-9 px-3 text-sm bg-white
                  ...
                  <div class="w-48 space-y-1">
                    ...
                    <select v-model="task.resolvedAssigneeId"
                      class="w-full border border-gray-300 rounded-lg h-9 px-2 text-sm bg-white
                             focus:outline-none focus:border-green-800 transition-colors">
                ...
                <div class="space-y-1">
                  ...
                  <textarea v-model="task.description" rows="2" maxlength="500"
                    placeholder="Describe the task…"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm resize-none bg-white
                           focus:outline-none focus:border-green-800 transition-colors" />
                </div>
                ...
                <div class="flex gap-3">
                  <div class="space-y-1">
                    ...
                    <select v-model="task.type"
                      class="border border-gray-300 rounded-lg h-9 px-2 text-sm bg-white
                  ...
                  <div class="flex-1 space-y-1">
                    ...
                    <input v-model="task.start" type="date"
                      class="w-full border border-gray-300 rounded-lg h-9 px-3 text-sm bg-white
                             focus:outline-none focus:border-green-800" />
                  </div>
                  <div class="flex-1 space-y-1">
                    ...
                    <input v-model="task.end" type="date"
                      class="w-full border border-gray-300 rounded-lg h-9 px-3 text-sm bg-white
                             focus:outline-none focus:border-green-800" />
                  </div>
                </div>
                ...
                <div class="space-y-2 pt-1">
                  <div class="flex items-center justify-between">
                    ...
                    <button type="button" @click="addSubTask(ti)"
                      class="text-xs text-green-800 font-semibold hover:text-green-600 transition-colors">
                      + Add sub-task
                    </button>
                  </div>
                  <div v-if="task.subTasks?.length" class="space-y-1.5">
                    <div v-for="(sub, si) in task.subTasks" :key="si" class="flex items-center gap-2">
                      ...
                      <input v-model="sub.name" type="text" maxlength="200"
                        :placeholder="`Sub-task ${si + 1}…`"
                        class="flex-1 border border-gray-200 rounded-lg h-8 px-3 text-xs bg-white
                               focus:outline-none focus:border-green-800" />
        ...
        <div v-else-if="activeTab === 'paste' && !rawInput"
          class="text-center py-10 text-gray-300 text-sm select-none">
      ...
      <div class="flex gap-3 px-7 py-4 border-t border-gray-100">
        ...
        <button
          @click="submitAll"
          :disabled="submitting || !parsedTasks.length"

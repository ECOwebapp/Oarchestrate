...
const _resolvePosName = (userId, allPositions, context = null) => {
  ...
  const names = finalRows
    .map(
      (row) =>
        posStore.position.find((p) => String(p.id) === String(row?.pos_id))
          ?.name,
    )
    .filter(Boolean);
...
const assignableMembers = computed(() => {
  ...
  if (newTask.value.design && (auth.isDirector || auth.isUnitHead)) {
    const juniorDraftsmanIds = new Set(
      allPositions
        .filter(
          (p) =>
            String(p.unit_id) === String(PDU_UNIT_ID) && Number(p.pos_id) === 5, // Junior Draftsman
        )
  ...
  if (auth.isDirector) {
    ...
    return allMembers
      ...
      .map((m) => ({
        ...m,
        pos_name: _resolvePosName(m.id, allPositions, DIRECTOR_ASSIGNABLE_NR),
      }));
  }

  if (auth.isUnitHead) {
    const unitId =
      auth.positions.find((p) => p.pos_id === POS_UNIT_HEAD)?.unit_id ?? null;
  ...
  if (auth.isSeniorDraftsman) {
    ...
    const juniorIds = allPositions
      .filter(
        (p) => Number(p.pos_id) === 5 && String(p.unit_id) === String(unitId),
      )
      .map((p) => String(p.user_id));
...
const submitForm = async () => {
  ...
  try {
    ...
    if (props.design && newTask.value.id) {
      if (!assigneeId)
        throw new Error("Please select an assignee for the design task.");
...
<template>
  ...
  >
    ...
    <div class="overflow-y-auto flex-1 px-7 py-5 space-y-4">
      ...
      >
        <svg
          class="w-4 h-4 text-green-700 mt-0.5 flex-shrink-0"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
        ...
        <p class="text-xs text-green-800">
          Pre-filled from the Director's original sub-task — you can edit any
          field before assigning.
        </p>
      </div>
      ...
      <div class="flex justify-start gap-10">
        ...
        <!-- Mark as Design -->
        <div
          v-if="newTask.type === 1 && (auth.isDirector || auth.isUnitHead)"
          class="flex items-center gap-2"
        >
          <input
            v-model="newTask.design"
            type="checkbox"
            id="design"
      ...
      >
        ...
        <p class="text-xs text-amber-700">
          ...
          <span v-else>
            This insertion task requires approval from your Unit Head and the
            Director.
          </span>
        </p>
      </div>

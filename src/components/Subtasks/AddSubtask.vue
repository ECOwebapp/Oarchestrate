...
const props = defineProps({
    ...
    taskHasAssignee: { type: Boolean, default: true },
});
        ...
        return allMembers
            ...
            .map((m) => ({
                ...
                pos_name: _resolvePosName(
                    m.id,
                    allPositions,
                    newTask.value.design ? [5, 6] : DIRECTOR_ASSIGNABLE_NR,
                ),
            }));
    }
            ...
            ...new Set(
                allPositions
                    .filter((p) => {
                        ...
                        // If design task, only show Senior Draftsman and Junior Draftsman
                        if (newTask.value.design) {
                            const isDesignPosition = DESIGN_POSITIONS.has(
                                String(p.pos_id),
                            );
        ...
        <div class="overflow-y-auto flex-1 px-7 py-5 space-y-4">
            ...
            >
                ...
                <p class="text-xs text-green-800">
                    Pre-filled from the original sub-task — you can edit any
                    field before saving.
                </p>
            </div>
            ...
            >
                ...
                <div
                    v-else-if="!props.taskHasAssignee"
                    class="rounded-xl px-3 py-2 text-xs font-medium text-red-500 flex gap-2 items-center"
                >
                    <svg
                    ...
                    You need to add an assignee to the parent task before you
                    can assign a user here.
                </div>

                <template v-else>
                    <select
                        v-model="newTask.assignee"
                        :disabled="!props.taskHasAssignee"
                        class="w-full border-2 border-gray-300 rounded-xl h-11 px-3 text-sm focus:outline-none focus:border-green-800 bg-white hover:cursor-pointer disabled:opacity-60"
                    >
                        <option :value="null" disabled hidden>
                            {{
                                props.taskHasAssignee
                                    ? "Select assignee..."
                                    : "Parent task has no assignee..."
                            }}
                        </option>
                        <option

...
<template>
    <div class="mb-5 flex flex-col gap-4">
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            <div
                class="flex min-w-0 flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"
            >
                <label
                    for="phone"
                ...
                <button
                    type="button"
                    @click="form.phone.push('')"
                    class="self-center rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-900 transition-all hover:-translate-y-0.5 hover:bg-emerald-100 hover:shadow-sm hover:cursor-pointer"
                >
                    Add phone
                </button>
            </div>
            ...
            >
                ...
                <div
                    v-for="(num, index) in form.email_address"
                    :key="index"
                    class="relative"
                >
                    <input
                        v-model="form.email_address[index]"
                        type="email"
                        id="email"
                        class="mb-4 w-full rounded-xl border border-slate-300 px-4 py-3 text-sm text-gray-700 shadow-[0_2px_8px_rgba(15,23,42,0.03)] transition focus:border-emerald-600 focus:text-black focus:outline-none focus:ring-2 focus:ring-emerald-100"
                    />
                ...
                <div
                    class="mb-4 bg-green-50 border border-green-200 text-green-900 text-xs rounded-lg px-4 py-3"
                >
                    Your primary email is your CarSU email to easily reset your
                    password.
                </div>

                <button
                    type="button"
                    @click="form.email_address.push('')"
                    class="self-center rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-900 transition-all hover:-translate-y-0.5 hover:bg-emerald-100 hover:shadow-sm hover:cursor-pointer"
                >
                    Add email
                </button>

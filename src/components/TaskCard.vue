...
<template>
  <div @click="handleClick" class="relative flex flex-col rounded-2xl py-3 px-3 sm:px-4 overflow-hidden bg-white shadow-lg
         ...
         animate-slide-up h-full min-h-[160px] sm:min-h-[176px]" :class="[
          cardClass,
    ...
    <div class="flex items-center gap-1.5 mb-2 flex-wrap">
      <span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full whitespace-nowrap"
        :class="task.type?.toLowerCase() === 'insertion' ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-800'">
        {{ task.type }}
      </span>
    ...
    <div class="flex items-center justify-between gap-2 mb-2">
      <p v-if="showAssignee" class="text-[10px] sm:text-xs text-gray-400 truncate flex items-center gap-1">
        <svg class="w-3 h-3 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
          <path :d="mdiAccount" />
        </svg>

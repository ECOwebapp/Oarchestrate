  ...
  <div>
    <div id="notif-widget" class="fixed bottom-16 right-6 z-50 flex flex-col items-end">
      ...
      <Transition name="panel">
        <div v-if="open" class="mb-3 w-84 bg-white rounded-2xl shadow-2xl border border-gray-100
          ...
          <div class="overflow-y-auto notif-scroll" style="max-height:480px">
            ...
            <div v-else class="py-2">
              <TransitionGroup name="notif-list" tag="div">
                <div v-for="(n, i) in notifStore.visible" :key="n.meta?.taskId ?? n.userId ?? i"
                    ...
                    <div class="px-3 pt-3 pb-2">
                      <div class="flex items-start gap-3">
                        <div class="relative flex-shrink-0">
                          ...
                          <span v-if="!n.read" class="absolute -top-0.5 -right-0.5 w-3 h-3 rounded-full bg-blue-500
                                 border-2 border-white" />
                        </div>
                        <div class="flex-1 min-w-0">
                          <div class="flex items-center justify-between gap-2 mb-1">
                            <span
                              class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 uppercase tracking-wide">
                              New Registration
                            </span>
                  ...
                  <template v-else-if="n.type === 'task_submitted'">
                    <button @click="openTask(n)" class="w-full text-left px-3 py-2.5 flex items-start gap-2.5
                           transition-colors duration-150 group" :class="n.meta?.urgent
                            ? 'hover:bg-red-100/60 cursor-pointer'
                            : 'hover:bg-green-100/60 cursor-pointer'">

                      <!-- Loading spinner overlay for this specific card -->
                        ...
                        <div class="flex-1 min-w-0">
                          <div class="flex items-center justify-between gap-2 mb-0.5">
                            ...
                            <div class="flex items-center gap-1.5 flex-shrink-0">
                              ...
                              <svg viewBox="0 0 24 24" class="w-3 h-3 fill-current text-gray-300 group-hover:text-green-600
      ...
      <button ref="bell" @click="toggle" class="relative w-12 h-12 rounded-2xl shadow-lg flex items-center justify-center hover:cursor-pointer
        ...
        <Transition name="badge">
          <span v-if="notifStore.unread > 0" class="absolute -top-1.5 -right-1.5 min-w-[20px] h-5 px-1 rounded-full
    ...
    <!-- ── Task Modal — mounted outside the widget so z-index is clean ── -->
    <TaskModal v-if="activeTask" :task="activeTask" @close="closeTask" @refresh="notifStore.fetchNotifs()" />

    <!-- Loading backdrop (shown while fetching task) -->
    <Transition name="panel">
    ...
    <!-- Error toast -->
    <Transition name="panel">
      <div v-if="loadTaskError" class="fixed bottom-32 right-6 z-50 bg-red-600 text-white text-xs font-bold

            ...
            <div v-else class="py-1.5 sm:py-2">
              <TransitionGroup name="notif-list" tag="div">
                <div v-for="(n, i) in notifStore.visible" :key="n.id || i"
                  ...
                  :class="[itemBg(n), !n.read ? 'shadow-sm' : 'opacity-70']" :style="`animation-delay:${i * 25}ms`">
                  ...
                  <template v-if="n.type === 'registration'">
                      ...
                      <div class="flex items-start gap-2.5 sm:gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                          <div class="flex items-center justify-between gap-2 mb-1">
                            <span
                              class="text-[10px] font-bold px-1.5 sm:px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 uppercase tracking-wide whitespace-nowrap">New
                              Registration</span>
                          ...
                          <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                            <span v-if="n.position" class="text-[10px] sm:text-xs text-gray-600 font-medium">{{
                              n.position }}</span>
                    ...
                    <div class="flex gap-1.5 sm:gap-2 px-2.5 sm:px-3 pb-2.5 sm:pb-3 pt-1">
                      <button @click="notifStore.denyUser(n.userId)" :disabled="isActing(n)"
                        class="hover:cursor-pointer flex-1 py-1.5 rounded-xl border-2 border-red-200 text-red-600 text-xs font-bold hover:bg-red-50 transition-all active:scale-95 disabled:opacity-40">Deny</button>
                      <button @click="notifStore.approveUser(n.userId)" :disabled="isActing(n)"
                        class="hover:cursor-pointer flex-1 py-1.5 rounded-xl bg-green-800 text-white text-xs font-bold hover:bg-green-700 transition-all active:scale-95 disabled:opacity-40">Approve</button>
                    </div>
                  </template>
                      ...
                      <template v-if="loadingTaskId === (n.meta?.taskId ?? n.taskId ?? n.id)">
                        <div class="flex-1 flex items-center justify-center py-1.5 gap-2 text-green-700">
                          <svg class="animate-spin w-3.5 h-3.5" viewBox="0 0 24 24" fill="none">
                            ...
                            <path d="M12 2a10 10 0 0 1 10 10" stroke="#15803d" stroke-width="3"
                  ...
                  <template v-else>
                    <div class="px-2.5 sm:px-3 py-2 sm:py-2.5 flex items-start gap-2 sm:gap-2.5">
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="flex items-center justify-between gap-2 mb-0.5">
                          <span class="text-[10px] sm:text-xs font-bold px-1.5 py-0.5 rounded-md"
                            :class="badgeClass(n)">{{ labelText(n) }}</span>
                        ...
                        <p class="text-[10px] sm:text-xs text-gray-500 mt-0.5 leading-snug line-clamp-2">{{ n.body }}
                        </p>
                      </div>
                    </div>
                  </template>

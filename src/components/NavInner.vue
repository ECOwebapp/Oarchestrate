...
<template>
  <div class="flex flex-col h-full w-full">
    <div
      ...
      :class="authStore.isLoggingOut ? 'opacity-60 pointer-events-none' : ''"
    >
      <div
      ...
      <div class="relative z-10 flex flex-col items-center w-full pb-4 px-3">
        ...
        <template v-if="!authStore.loading && authStore.initialized">
          <div v-if="expanded" class="text-center">
            <p
              class="text-white text-sm font-bold leading-tight truncate max-w-[180px]"
            >
              {{ authStore.fullName }}
            </p>
            <p
              class="text-green-300 text-[13px] font-bold truncate mt-0.5 max-w-[180px]"
            >
              {{ formattedPositions }}
            </p>
        ...
        <div v-else class="flex flex-col items-center gap-1.5">
          ...
          <div
            v-show="expanded"
            class="h-2.5 w-20 rounded bg-white/15 animate-pulse"
          ></div>
        </div>
      </div>
    </div>
    ...
    <ul
      class="flex flex-col flex-1 py-2 overflow-hidden"
      :class="authStore.isLoggingOut ? 'opacity-60 pointer-events-none' : ''"
    >
      <li v-for="(name, i) in navNames" :key="name">
        <router-link
          ...
          "
          :style="{
            animation: `navIn 0.35s cubic-bezier(.16,1,.3,1) both`,
        ...
        >
          ...
          <Icons
            ...
            class="flex-shrink-0 transition-transform duration-150 mx-4"
    ...
    <div class="flex-shrink-0 border-t border-white/10 p-2">
      <button
        ...
        class="group relative w-full flex flex-row items-center rounded-xl py-3 mx-0 overflow-hidden text-red-600 hover:bg-red-500/30 hover:cursor-pointer transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
      >
        <Icons
          v-if="authStore.isLoggingOut"
          icon="spin"
          class="flex-shrink-0 mx-4 animate-spin"
        />
        <Icons
          v-else
          icon="logout"
        ...
        />
        <span v-if="expanded" class="text-sm font-medium mr-4">
          {{ authStore.isLoggingOut ? "Logging out..." : "Logout" }}
        </span>
      </button>
      <p
        v-if="expanded"
        class="text-center text-[10px] text-green-700 mt-1.5 tracking-widest"
      >
        v{{ version }}
      </p>
    </div>

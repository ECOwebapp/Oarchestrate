...
const mdiCloudUpload = 'M11 20H6.5Q4.22 20 2.61 18.43 1 16.85 1 14.58 1 12.63 2.17 11.1 3.35 9.57 5.25 9.15 5.88 6.85 7.75 5.43 9.63 4 12 4 14.93 4 16.96 6.04 19 8.07 19 11 20.73 11.2 21.86 12.5 23 13.78 23 15.5 23 17.38 21.69 18.69 20.38 20 18.5 20H13V12.85L14.6 14.4L16 13L12 9L8 13L9.4 14.4L11 12.85Z'
...
const statusLabel = computed(() => {
  ...
  if (props.task?.assigneeIsOffice && props.task?.outputLink)
    return { label: 'Pending Director Review', cls: 'bg-amber-100 text-amber-800', icon: mdiClockOutline }
  return { label: 'Pending Approval', cls: 'bg-gray-100 text-gray-600', icon: mdiClockOutline }
})
...
<template>
  <div v-if="task" class="fixed inset-0 z-100 flex items-end sm:items-center justify-center bg-black/50 px-0 sm:px-4"
    ...
    <div class="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl
      ...
      <!-- Close button -->
      <button class="absolute top-3.5 right-4 w-8 h-8 flex items-center justify-center rounded-full
               text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors z-10" @click="emit('close')">
      ...
      <div class="px-6 sm:px-8 pt-7 pb-4 flex-shrink-0">
        <div class="flex flex-wrap gap-2 mb-3">
          ...
          <span v-if="task.revision" class="px-3 py-1 text-xs font-bold rounded-full bg-purple-600 text-white">
            Revision
          </span>
          <span class="px-3 py-1 text-xs font-bold rounded-full flex items-center gap-1" :class="statusLabel.cls">
            <svg viewBox="0 0 24 24" class="w-3 h-3 flex-shrink-0" fill="currentColor">
              <path :d="statusLabel.icon" />
            </svg>
            {{ statusLabel.label }}
          </span>
          <span v-if="isOverdue" class="px-3 py-1 text-xs font-bold rounded-full bg-red-100 text-red-700">
            ⚠ Overdue
          </span>
        </div>
      ...
      <div class="flex border-b border-gray-100 px-6 sm:px-8 flex-shrink-0 gap-1">
        <button v-for="t in ['detail', 'comments']" :key="t" @click="tab = t"
          ...
          <span v-if="tab === t" class="absolute bottom-0 left-0 right-0 h-0.5 bg-green-900 rounded-full" />
        </button>
      </div>
          ...
          <div>
            ...
            <div v-else-if="canSubmitOutput" class="space-y-2.5">
              ...
              <div v-if="!uploadFile" @dragover.prevent="dragOverSubmit = true"
                @dragleave.prevent="dragOverSubmit = false" @drop.prevent="onDropSubmit" @click="fileInputRef?.click()"
                class="border-2 border-dashed rounded-2xl px-5 py-7 flex flex-col items-center
                       justify-center gap-2 cursor-pointer transition-all select-none" :class="dragOverSubmit ? 'border-green-700 bg-green-50'
            ...
            <div v-else-if="canResubmit" class="space-y-2.5">
              ...
              <div v-if="!resubmitFile" @dragover.prevent="dragOverResub = true"
                @dragleave.prevent="dragOverResub = false" @drop.prevent="onDropResub" @click="resubInputRef?.click()"
                class="border-2 border-dashed rounded-2xl px-5 py-7 flex flex-col items-center
              ...
              <button @click="resubmit" :disabled="acting === 'resubmit' || !resubmitFile" class="w-full h-11 rounded-xl bg-orange-600 text-white text-sm font-bold

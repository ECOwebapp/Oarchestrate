...
<template>
  <div class="flex-1 min-h-0 overflow-auto relative bg-white">
    ...
    <div
      class="absolute inset-0 pointer-events-none opacity-80 bg-[url('/images/csu-background')] bg-cover bg-center">
    </div>
    ...
    <div v-if="!showManagement" class="relative z-10 flex flex-col items-center py-2 px-2 w-full">
      ...
      <div class="w-full flex justify-between items-center mb-1 px-2">
        <div class="flex items-center gap-3">
          <img src="../../public/images/csu_logo.png" alt="CSU"
        ...
        <div class="flex items-center gap-2">
          <button @click="showManagement = true" 
                  class="text-white font-bold text-sm px-10 py-4 rounded-3xl shadow-lg hover:bg-green-900 transition-colors"
      ...
      <div class="w-full flex items-start gap-2 px-2">
        ...
        <div class="flex-1 flex flex-col items-center min-w-0">
          ...
          <div class="flex justify-center gap-1 w-full flex-wrap">
            <OrgCard v-for="m in orgData.pdu.members" :key="m.name"
                      v-bind="m" :sz="sz.sm" />
          </div>
          <VLine />
          ...
          <div class="flex flex-col items-center gap-0">
            ...
            <div class="flex justify-center gap-1 flex-wrap">
              <div v-for="s in orgData.pdu.subMembers.slice(1)" :key="s.name"
        ...
        <div class="flex-1 flex flex-col items-center min-w-0">
          ...
          <div class="flex gap-1 flex-wrap">
            <div v-for="e in orgData.piu.siteEngineers" :key="e.name"
                 class="flex flex-col items-center">
              <VLine />
          ...
          <div class="flex justify-center gap-1 w-full flex-wrap">
            <OrgCard v-for="s in orgData.piu.subMembers" :key="s.name"
               v-bind="s" :sz="sz.sm" />
          </div>
        </div>
      </div>
    </div> <div v-else class="relative z-10 flex flex-col p-8">
      <button 
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-6xl mx-auto w-full">
        ...
        <div class="bg-white rounded-lg p-6 shadow-md border border-gray-100">
          ...
          <div class="space-y-4">
            <div v-for="member in pendingMembers" :key="member.name" class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              <div class="flex items-center gap-3">
              ...
              <div class="flex gap-2">
                <button class="px-4 py-1 bg-green-700 text-white text-xs font-bold rounded-lg hover:bg-green-800 transition-colors">Approve</button>
                <button class="px-4 py-1 border-2 border-red-500 text-red-500 text-xs font-bold rounded-lg hover:bg-red-50 transition-colors">Decline</button>
              </div>
            </div>
        ...
        <div class="flex flex-col gap-8">
          <div class="bg-white rounded-lg p-6 shadow-md border border-gray-100">
            ...
            <div class="space-y-4">
              <div>
                ...
                <select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-700">
                  <option disabled selected value="">-- Select a member --</option>
                  <option>Jacky Villarin</option>
                  <option>Nheron Cedro</option>
                  <option>Austin Rey</option>
                  <option>Cyrel John Rollo</option>
                  <option>June Luis Bermudez</option>
                </select>
              ...
              <button class="w-full mt-2 px-4 py-2 bg-green-700 text-white font-bold rounded-lg hover:bg-green-800 transition-colors">Confirm</button>
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg p-6 shadow-md border border-gray-100">
            ...
            <div class="space-y-4">
              ...
              <button class="w-full mt-4 px-4 py-2 bg-red-700 text-white font-bold rounded-lg hover:bg-red-800 transition-colors">Remove</button>
            </div>
          </div>
        </div>
      </div>

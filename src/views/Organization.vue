...
const chartUsers = computed(() => {
  ...
  return positions.memberPos
    ...
    .map((link) => {
      ...
      const mi = member.middle_initial?.trim()
...
const orgData = computed(() => {
  ...
  const getHead = (unitId) =>
    users.find((u) => u.unitId === unitId && u.title === "Unit Head");
  ...
  const consolidateMembers = (unitId) => {
    ...
    const unitUsers = chartUsers.value.filter(
      (u) => u.unitId === unitId && u.title !== "Unit Head",
    );
  ...
  return {
    ...
    divChief: {
      ...
      avatarUrl: users.find((u) => u.title === "Director")?.avatarUrl,
    },

    // 2. Dynamic DB Data
    director: users.find((u) => u.title === "Director") || {
      name: "Vacant",
      title: "Director",
    },
    ...
    pdu: {
      ...
      members: consolidateMembers(1).filter(
        (u) =>
          u.title !== "Unit Head" && !u.title.toLowerCase().includes("drafts"),
      ),
      // Sub-members specifically looking for Draftsman/woman in Unit 1
      seniorDrafts: pduAll.find((u) =>
        u.title.toLowerCase().includes("senior"),
      ),
      juniorDrafts: pduAll.filter((u) =>
        u.title.toLowerCase().includes("junior"),
      ),
    },
    ...
    piu: {
      ...
      siteEngineers: piuAll.filter((u) =>
        u.title.toLowerCase().includes("site engineer"),
      ),
    },
  };
});

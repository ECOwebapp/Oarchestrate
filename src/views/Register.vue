...
onMounted(async () => {
  ...
  const r = await fetch(`${PSGC}/regions/`).then(r => r.json()).catch(() => [])
...
<template>
  <div class="relative min-h-screen w-full overflow-hidden flex items-center justify-center">
    ...
    <!-- Card -->
    <div class="relative z-10 w-full max-w-lg mx-4 bg-white/95 backdrop-blur rounded-2xl shadow-2xl px-10 py-8 my-6">
      ...
      <div class="flex items-center mb-6 px-2">
        ...
        <div class="flex-1 h-0.5 transition-all duration-300" :class="step > 1 ? 'bg-green-700' : 'bg-gray-200'">
        </div>
        ...
        <div class="flex items-center gap-2">
          <div
            ...
            <svg v-if="step > 2" viewBox="0 0 14 14" fill="none" class="w-3.5 h-3.5">
              <path d="M2.5 7l3 3 6-6" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
            <span v-else>2</span>
          </div>
          <span class="text-xs font-semibold uppercase tracking-wider"
            :class="step >= 2 ? 'text-green-800' : 'text-gray-400'">Address</span>
        </div>
        <div class="flex-1 h-0.5 transition-all duration-300" :class="step > 2 ? 'bg-green-700' : 'bg-gray-200'">
        </div>

        <div class="flex items-center gap-2">
          <div
            class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all duration-300"
            :class="step === 3 ? 'bg-green-800 border-green-800 text-white' : 'border-gray-300 text-gray-400'">
            <span>3</span>
          </div>
          <span class="text-xs font-semibold uppercase tracking-wider"
            :class="step === 3 ? 'text-green-800' : 'text-gray-400'">Credentials</span>
        </div>
      </div>

      <!-- General error -->
      <div v-if="errors.general" class="mb-4 bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-3">
        {{ errors.general }}
      </div>
      ...
      <div v-show="step === 1" class="space-y-4">
        ...
        <div class="flex gap-2">
          <div class="flex-1">
            <input v-model="form.firstName" type="text" placeholder="First Name" @input="clearError('firstName')"
          ...
          <div class="flex-1">
            <input v-model="form.lastName" type="text" placeholder="Last Name" @input="clearError('lastName')"
              class="w-full px-4 py-3 rounded-lg border text-gray-700 bg-white text-sm placeholder-gray-400 transition focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-transparent"
        ...
        <!-- ID Number -->
        <div>
          <input v-model="form.idNumber" type="text" placeholder="ID Number" @input="clearError('idNumber')"
            class="w-full px-4 py-3 rounded-lg border text-gray-700 bg-white text-sm placeholder-gray-400 transition focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-transparent"
        ...
        <div class="flex gap-2">
          <div class="flex-1">
            <select v-model="form.genderId" @change="clearError('genderId')"
              ...
              <option v-for="g in genders" :key="g.id" :value="g.id">{{ g.type }}</option>
            </select>
        ...
        <div class="rounded-xl border border-gray-200 overflow-hidden">
          ...
          <!-- Unit -->
          <p class="text-sm text-gray-600 px-2 py-1">Add your position in <em>Profile > Work Information</em>.</p>
        ...
        <button type="button" @click="nextStep"
          ...
          <svg viewBox="0 0 20 20" fill="none" class="w-4 h-4">
            <path d="M4 10h12M11 5l5 5-5 5" stroke="white" stroke-width="1.8" stroke-linecap="round"
      ...
      <div v-show="step === 2" class="space-y-4">
        ...
        <div class="space-y-2">
          ...
          <div v-if="form.regionCode && !isNCR">
            ...
            <select v-else v-model="form.provinceCode" @change="clearError('provinceCode')" :disabled="!form.regionCode"
          ...
          <div v-if="form.cityCode">
            ...
            <select v-else v-model="form.barangayCode" @change="clearError('barangayCode')" :disabled="!form.cityCode"
              class="hover:cursor-pointer w-full px-4 py-3 rounded-lg border text-gray-700 bg-white text-sm transition focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-transparent disabled:bg-gray-50 disabled:text-gray-400"
      ...
      <div v-show="step === 3" class="space-y-4">
        ...
        <!-- Phone -->
        <div>
          <input v-model="form.phone" type="tel" placeholder="Phone Number" @input="clearError('phone')"
            class="w-full px-4 py-3 rounded-lg border text-gray-700 bg-white text-sm placeholder-gray-400 transition focus:outline-none focus:ring-2 focus:ring-green-700 focus:border-transparent"
        ...
        <div>
          ...
          <div v-if="form.password" class="hover:cursor-pointer flex items-center gap-2 mt-1.5">
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 text-center">
          ...
          <div class="w-20 h-20 mx-auto mb-5">
            <svg viewBox="0 0 80 80" fill="none" class="w-full h-full">
              <circle cx="40" cy="40" r="36" stroke="#15803d" stroke-width="3" stroke-dasharray="226"
                stroke-dashoffset="226" style="animation: drawCircle 0.6s ease forwards;" />
              <path d="M24 40l10 10 22-22" stroke="#15803d" stroke-width="4" stroke-linecap="round"
                stroke-linejoin="round" stroke-dasharray="48" stroke-dashoffset="48"
                style="animation: drawCheck 0.4s ease 0.55s forwards;" />
            </svg>
          </div>

          <h2 class="text-2xl font-bold text-gray-800 mb-2" style="font-family: 'Georgia', serif;">

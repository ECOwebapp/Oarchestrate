...
const handleRegister = async () => {
  ...
  try {
    ...
    await supabase.from('contact').upsert({ user_id: userId, phone: form.phone.trim() })
    ...
    await supabase.from('unit').upsert({ user_id: userId, unit_id: parseInt(form.unitId) })
    await supabase.from('position').upsert({ user_id: userId, pos_id: parseInt(form.positionId) })
    await supabase.from('member_type').upsert({ user_id: userId, role_id: 3 })

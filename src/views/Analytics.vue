...
const lineRaw = computed(() => {
  ...
  analyticsTasks.value.filter(t => t.director && taskYear(t) === yr)
...
const areaSeriesData = computed(() => {
  ...
  analyticsTasks.value.filter(t => taskYear(t) === yr).forEach(t => {
...
<template>
  <div class="overflow-auto">
        ...
        <div class="relative flex min-h-[260px] flex-col rounded-xl bg-white p-4 shadow-sm md:h-full md:min-h-0">
          ...
          <div class="min-h-0 flex-1 overflow-hidden">
            <svg viewBox="0 0 500 245" class="w-full h-full">
              ...
              <!-- X labels -->
              <text v-for="b in barRects" :key="'lbl' + b.label" :x="b.cx" y="217" text-anchor="middle"
                font-size="9" fill="#6b7280">{{ b.label }}</text>
              <!-- X axis title -->
              <text x="265" y="234" text-anchor="middle" font-size="10" font-weight="bold"
                fill="#374151">Tasks</text>
            </svg>
          </div>
        </div>
          ...
          <div class="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 sm:flex-row sm:gap-6">
            <svg viewBox="0 0 200 200" class="h-full max-h-44 w-full max-w-[220px] flex-shrink-0 sm:max-h-52">
              ...
              <template v-for="seg in pieSegments" :key="'p'+seg.label">
                <text v-if="seg.count > 0" :x="seg.lx" :y="seg.ly + 6" text-anchor="middle" font-size="8"
                  fill="white" class="pointer-events-none">
                  ({{ seg.pct }}%)
                </text>
              </template>
            </svg>
            <div class="flex w-full flex-col gap-2 sm:w-auto sm:gap-2.5">
              <div v-for="d in pieData" :key="d.label"
        ...
        <div class="relative flex min-h-[260px] flex-col rounded-xl bg-white p-4 shadow-sm md:h-full md:min-h-0">
          ...
          <div class="min-h-0 flex-1 overflow-hidden">
            <svg viewBox="0 0 500 215" class="w-full h-full">
              ...
              <!-- Dots -->
              <circle v-for="(p, i) in linePoints" :key="i" :cx="p.x" :cy="p.y" r="3" fill="#16a34a"
                stroke="#16a34a" stroke-width="2" fill-opacity="0.35" />
              <!-- X labels -->
          ...
          <div class="min-h-0 flex-1 overflow-hidden">
            <svg viewBox="0 0 500 200" class="w-full h-full">
              ...
              <text v-for="t in areaTickPos" :key="'ay' + t.v" x="30" :y="t.y + 3" text-anchor="end" font-size="8"
                fill="#9ca3af">{{ t.v }}</text>
              <!-- Area fills -->
              <path v-for="s in areaSeries" :key="'a' + s.key" :d="s.areaPath" :fill="s.color"
                fill-opacity="0.22" />
              <!-- Lines on top -->
    ...
    <IndividualAccomplishmentReport :show="showIndividualReport" :month="indivMonth" :year="indivYear"
      :dateFrom="indivDateFrom" :dateTo="indivDateTo" :userName="auth.fullName"
      @close="showIndividualReport = false" />

    <!-- ── Expanded Chart View (component) ── -->
    <AnalyticsChart :expandedChart="expandedChart" :barRects="barRects" :barData="barData"
      :pieSegments="pieSegments" :pieData="pieData" :linePolyline="linePolyline" :linePoints="linePoints"
      :lineRaw="lineRaw" :monthLabels="monthLabels" :areaSeries="areaSeries" :areaSeriesData="areaSeriesData"
      :barTickPos="barTickPos" :lineTickPos="lineTickPos" :areaTickPos="areaTickPos" @close="closeModal" />

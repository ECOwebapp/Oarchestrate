...
const pieData = computed(() => {
  ...
  return Object.entries(counts).map(([label, count]) => ({
    label, count, pct: Math.round((count / total) * 100), color: STATUS_COLORS[label],
  }))
})
...
<template>
  <!--
    Layout:
    - Mobile/tablet  (<lg): full-width vertical stack, both panels scroll naturally
    - lg+:                  side-by-side, left panel fixed 256px, right panel fills remaining, both scroll-locked
    The outer wrapper is scroll-enabled on mobile, overflow-hidden on lg (each panel handles its own scroll).
  -->
  <div class="flex h-full min-h-0 w-full flex-col gap-4 overflow-y-auto overflow-x-hidden
              bg-gray-50 p-3 sm:p-4 lg:flex-row lg:overflow-hidden">

    <!-- ── Left: Accomplishment Report Generator ── -->
    <div class="w-full rounded-xl bg-white p-4 shadow-sm sm:p-5
                lg:h-full lg:w-64 lg:flex-shrink-0 lg:flex lg:flex-col lg:overflow-hidden">

      <div class="flex-shrink-0">
        <h2 class="text-sm sm:text-base font-extrabold text-gray-900 leading-snug mb-2 sm:mb-3">
          Accomplishment<br class="hidden sm:block" /> Report Generator
        </h2>
      ...
      <!-- Notes: collapse to a summary on mobile to save vertical space -->
      <div class="mt-3 lg:flex-1 lg:overflow-y-auto">
        <details class="lg:open" open>
          <summary class="text-xs font-semibold text-gray-700 mb-1 cursor-pointer list-none
                          flex items-center gap-1 lg:cursor-default select-none">
            <svg class="w-3 h-3 text-gray-400 details-arrow lg:hidden" viewBox="0 0 24 24" fill="currentColor">
              <path d="M7 10l5 5 5-5z"/>
            </svg>
            Note:
          </summary>
          <ul class="text-xs text-gray-600 space-y-2 list-disc list-outside pl-4 mt-1">
      ...
      <!-- Buttons: side-by-side on mobile (space-efficient), stacked on lg -->
      <div class="mt-3 sm:mt-4 grid grid-cols-2 gap-2 lg:grid-cols-1 lg:mt-auto lg:flex-shrink-0">
        <button
          class="flex w-full items-center justify-center gap-1.5 sm:gap-2 rounded-lg
                 bg-green-950 px-3 py-2.5 sm:py-3 text-xs font-semibold text-white
                 shadow-sm transition-all cursor-pointer hover:bg-green-800 active:scale-95"
        ...
        <button
          class="flex w-full items-center justify-center gap-1.5 sm:gap-2 rounded-lg
                 border-2 border-green-950 bg-white px-3 py-2.5 sm:py-3 text-xs font-semibold
                 text-green-950 shadow-sm transition-all cursor-pointer hover:bg-green-50 active:scale-95"
    ...
    <!--
      Mobile:  single-column stack, each card has a min-height so charts are readable
      md:      2-column, 2-row grid; natural height
      lg+:     2-column, 2-row grid, fills remaining height with overflow-hidden per card
    -->
    <div class="grid w-full grid-cols-1 gap-3 sm:gap-4
                md:grid-cols-2
                lg:min-h-0 lg:flex-1 lg:grid-rows-2 lg:overflow-hidden">

      <!-- ── Bar Chart: Completion Rate ── -->
      <div class="relative flex min-h-[240px] sm:min-h-[260px] flex-col rounded-xl bg-white p-3 sm:p-4 shadow-sm
                  lg:h-full lg:min-h-0">
        <div class="mb-2 flex items-start justify-between gap-2 flex-shrink-0">
          <h3 class="text-xs font-semibold text-gray-700 sm:text-sm">Completion Rate</h3>
          <button class="text-gray-300 hover:text-gray-500 transition-colors cursor-pointer flex-shrink-0"
                  @click="openModal('bar')">
        ...
        <div class="min-h-0 flex-1 overflow-hidden">
          <svg viewBox="0 0 500 245" class="w-full h-full">
            ...
            <rect v-for="b in barRects" :key="b.label" :x="b.x" :y="b.y" :width="b.barW" :height="b.height" :fill="b.color" rx="2" />
            <text v-for="b in barRects" :key="'cnt'+b.label" :x="b.cx" :y="b.count > 0 ? b.y - 5 : 196" text-anchor="middle" font-size="9" font-weight="600" fill="#374151">{{ b.count }}</text>
            <text v-for="b in barRects" :key="'lbl'+b.label" :x="b.cx" y="217" text-anchor="middle" font-size="9" fill="#6b7280">{{ b.label }}</text>
      ...
      <div class="relative flex min-h-[240px] sm:min-h-[260px] flex-col rounded-xl bg-white p-3 sm:p-4 shadow-sm
        ...
        <div class="flex min-h-0 flex-1 flex-col items-center justify-center gap-3 sm:flex-row sm:gap-6">
          <svg viewBox="0 0 200 200"
               class="h-full max-h-36 sm:max-h-44 lg:max-h-52 w-full max-w-[180px] sm:max-w-[220px] flex-shrink-0">
            ...
            <template v-for="seg in pieSegments" :key="'v'+seg.label">
              <text v-if="seg.count > 0" :x="seg.lx" :y="seg.ly - 4" text-anchor="middle" font-size="9" font-weight="bold" fill="white" class="pointer-events-none">{{ seg.count }}</text>
            </template>
            <template v-for="seg in pieSegments" :key="'p'+seg.label">
              <text v-if="seg.count > 0" :x="seg.lx" :y="seg.ly + 6" text-anchor="middle" font-size="8" fill="white" class="pointer-events-none">({{ seg.pct }}%)</text>
            </template>
          </svg>
          <!-- Legend: horizontal on mobile, vertical on sm+ -->
          <div class="flex flex-row flex-wrap justify-center gap-x-3 gap-y-1.5 sm:flex-col sm:gap-2 sm:gap-2.5">
            <div v-for="d in pieData" :key="d.label" class="flex items-center gap-1.5 text-xs text-gray-700">
      ...
      <!-- ── Line Chart: Approved tasks trend ── -->
      <div class="relative flex min-h-[240px] sm:min-h-[260px] flex-col rounded-xl bg-white p-3 sm:p-4 shadow-sm
                  lg:h-full lg:min-h-0">
        <div class="mb-2 flex items-start justify-between gap-2 flex-shrink-0">
        ...
        <div class="min-h-0 flex-1 overflow-hidden">
          <svg viewBox="0 0 500 215" class="w-full h-full">
            ...
            <circle v-for="(p, i) in linePoints" :key="i" :cx="p.x" :cy="p.y" r="3" fill="#16a34a" stroke="#16a34a" stroke-width="2" fill-opacity="0.35" />
            <text v-for="(m, i) in monthLabels" :key="m" :x="35 + (i / 11) * 455" y="200" text-anchor="middle" font-size="8" fill="#6b7280">{{ m }}</text>
          </svg>
        </div>
      </div>
      ...
      <div class="relative flex min-h-[240px] sm:min-h-[260px] flex-col rounded-xl bg-white p-3 sm:p-4 shadow-sm
                  lg:h-full lg:min-h-0">
        <div class="mb-2 flex items-start justify-between gap-2 flex-shrink-0">
          <h3 class="text-xs font-semibold text-gray-700 sm:text-sm leading-snug">
            Submitted, Pending &amp; Revision Monthly Trend
          </h3>
          <button class="text-gray-300 hover:text-gray-500 transition-colors cursor-pointer flex-shrink-0"
        ...
        <div class="min-h-0 flex-1 overflow-hidden">
          <svg viewBox="0 0 500 200" class="w-full h-full">
            ...
            <template v-for="s in areaSeries" :key="'d'+s.key">
              <circle v-for="(p, i) in s.pts" :key="i" :cx="p.x" :cy="p.y" r="3" fill="white" :stroke="s.color" stroke-width="1.5" stroke-opacity="0.35" />
            </template>
            <text v-for="(m, i) in monthLabels" :key="m" :x="35 + (i / 11) * 455" y="190" text-anchor="middle" font-size="8" fill="#6b7280">{{ m }}</text>
          </svg>
        </div>
        <!-- Legend -->
        <div class="mt-1 flex flex-wrap justify-center gap-2 sm:gap-3 pb-0.5 text-xs text-gray-700 flex-shrink-0">
          <div v-for="s in areaSeriesData" :key="s.key" class="flex items-center gap-1.5">
            <span class="w-4 h-2 sm:w-5 sm:h-2.5 rounded-sm flex-shrink-0" :style="{ background: s.color }"></span>
            {{ s.key }}
          </div>
        </div>
      </div>
  ...
  <ReportPicker v-if="showUnitPicker"       title="Generate Unit Report"       :payroll="true"
    @cancel="showUnitPicker = false"         @generate="onGenerateUnitReport" />
  ...
  <AccomplishmentReport
    :show="showReport" :month="unitMonth" :year="unitYear"
    :dateFrom="unitDateFrom" :dateTo="unitDateTo"
    @close="showReport = false" />
  <IndividualAccomplishmentReport
    :show="showIndividualReport" :month="indivMonth" :year="indivYear"
    :dateFrom="indivDateFrom" :dateTo="indivDateTo"
    :userName="auth.fullName"
    @close="showIndividualReport = false" />
...
<style scoped>
/* Rotate the summary arrow when <details> is open */

    ...
    <div class="flex h-full min-h-0 w-full flex-col gap-4 overflow-y-auto overflow-x-hidden
      ...
      <div class="w-full rounded-xl bg-white p-4 shadow-sm sm:p-5
        ...
        <div class="mt-3 sm:mt-4 grid grid-cols-2 gap-2 lg:grid-cols-1 lg:mt-auto lg:flex-shrink-0">
          <button class="flex w-full items-center justify-center gap-1.5 sm:gap-2 rounded-lg
            ...
            <svg viewBox="0 0 200 200"
              ...
              <template v-for="seg in pieSegments" :key="'v'+seg.label">
                <text v-if="seg.count > 0" :x="seg.lx" :y="seg.ly - 4" text-anchor="middle" font-size="9"
                  font-weight="bold" fill="white" class="pointer-events-none">{{ seg.count }}</text>
              </template>
              <template v-for="seg in pieSegments" :key="'p'+seg.label">
                <text v-if="seg.count > 0" :x="seg.lx" :y="seg.ly + 6" text-anchor="middle" font-size="8" fill="white"
                  class="pointer-events-none">({{ seg.pct }}%)</text>
              </template>
            </svg>

    ...
    <div class="flex w-full h-full bg-gray-50 p-4 gap-4 overflow-auto">
        ...
        <div class="mt-auto">
            <button
            class="w-full py-2.5 bg-green-950 text-white text-sm font-bold rounded-lg hover:bg-green-800 transition-colors">
            Generate Report
            </button>
        </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 flex flex-col relative">
            ...
            <button class="absolute bottom-3 right-3 text-gray-300 hover:text-gray-500 transition-colors" @click="openModal('bar')">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 flex flex-col relative">
            ...
            <button class="absolute bottom-3 right-3 text-gray-300 hover:text-gray-500 transition-colors" @click="openModal('pie')">
            <Icons icon="fullscreen" class="w-4 h-4" />
            </button>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 flex flex-col relative">
            ...
            <svg viewBox="0 0 500 215" class="w-full h-full">
                ...
                <!-- Grid lines (y=185-(v/1100)*175) for 250,500,750,1000 -->
            ...
            <button class="absolute bottom-3 right-3 text-gray-300 hover:text-gray-500 transition-colors" @click="openModal('line')">
            <Icons icon="fullscreen" class="w-4 h-4" />
            </button>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 flex flex-col relative">
            ...
            <svg viewBox="0 0 500 200" class="w-full h-full">
                ...
                <!-- Grid lines (y=175-(v/1100)*165) for 250,500,750,1000 -->
            ...
            <button class="absolute bottom-7 right-3 text-gray-300 hover:text-gray-500 transition-colors" @click="openModal('area')">
            <Icons icon="fullscreen" class="w-4 h-4" />
            </button>
        </div>
    ...
    <div v-if="expandedChart" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-[80vw] h-[80vh] max-w-5xl mx-4 flex flex-col overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b flex-shrink-0">
            ...
            <button class="text-2xl text-gray-400 hover:text-gray-700 leading-none" @click="closeModal">×</button>
        </div>
        ...
        <div class="flex-1 min-h-0 p-6 flex items-center justify-center">
            ...
            <text x="35" y="62"  text-anchor="end" font-size="9" fill="#9ca3af">75</text>
            <text x="35" y="14"  text-anchor="end" font-size="9" fill="#9ca3af">100</text>
            <rect v-for="b in barRects" :key="b.label" :x="b.x" :y="b.y" width="65" :height="b.height" :fill="b.color" rx="2" />
            <text v-for="b in barRects" :key="'lbl'+b.label" :x="b.x + 32" y="217" text-anchor="middle" font-size="9" fill="#6b7280">{{ b.label }}</text>
            <text x="265" y="234" text-anchor="middle" font-size="10" font-weight="bold" fill="#374151">Tasks</text>
            </svg>
            ...
            <svg viewBox="0 0 200 200" class="h-full max-h-150">
                <path v-for="seg in pieSegments" :key="seg.label" :d="seg.path" :fill="seg.color" stroke="white" stroke-width="1.5" />
                <text v-for="seg in pieSegments" :key="'v'+seg.label" :x="seg.lx" :y="seg.ly - 4" text-anchor="middle" font-size="9" font-weight="bold" fill="white">{{ seg.value }}</text>
                <text v-for="seg in pieSegments" :key="'p'+seg.label" :x="seg.lx" :y="seg.ly + 6" text-anchor="middle" font-size="8" fill="white">({{ seg.value }}%)</text>
            </svg>
            ...
            <svg viewBox="0 0 500 200" class="w-full flex-1 min-h-0">
                ...
                <text x="30" y="66"  text-anchor="end" font-size="8" fill="#9ca3af">750</text>
                <text x="30" y="28"  text-anchor="end" font-size="8" fill="#9ca3af">1000</text>
                <path v-for="s in areaSeries" :key="'a'+s.key" :d="s.areaPath" :fill="s.color" fill-opacity="0.2" />
                <path v-for="s in areaSeries" :key="'l'+s.key" :d="s.linePath" fill="none" :stroke="s.color" stroke-width="2" stroke-opacity="0.35" />
                <template v-for="s in areaSeries" :key="'d'+s.key">
                <circle v-for="(p, i) in s.pts" :key="i"
                    :cx="p.x" :cy="p.y" r="3" :fill="s.color" :stroke="s.color" stroke-width="1.5" fill-opacity="0.35" stroke-opacity="0.35" />
                </template>

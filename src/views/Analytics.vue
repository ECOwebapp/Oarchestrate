...
const barCounts = computed(() => {
  ...
  analyticsTasks.value.forEach((t) => {
    const s = statusOf(t);
...
const barTicks = computed(() =>
  niceTicks(Math.max(...Object.values(barCounts.value), 0)),
);
const barTickTop = computed(
  () => barTicks.value[barTicks.value.length - 1] || 1,
);
const barTickPos = computed(() =>
  barTicks.value.map((v) => ({
    v,
    y: Math.round(200 - (v / barTickTop.value) * 190),
  })),
);
...
const pieHasData = computed(() => analyticsTasks.value.length > 0);
...
const lineRaw = computed(() => {
  ...
  analyticsTasks.value
    .filter((t) => t.director && taskYear(t) === yr)
    .forEach((t) => {
      const m = taskMonth(t);
      if (m >= 0) counts[m]++;
    });
...
const lineTickTop = computed(
  () => lineTicks.value[lineTicks.value.length - 1] || 1,
);
const lineTickPos = computed(() =>
  lineTicks.value.map((v) => ({
    v,
    y: Math.round(185 - (v / lineTickTop.value) * 175),
  })),
);
...
const areaSeriesData = computed(() => {
  ...
  Object.keys(def).forEach((k) => {
    buckets[k] = Array(12).fill(0);
  });
  analyticsTasks.value
    .filter((t) => taskYear(t) === yr)
    .forEach((t) => {
      const m = taskMonth(t);
      if (m < 0) return;
      const s = statusOf(t);
  ...
  return Object.entries(def).map(([key, color]) => ({
    key,
    color,
    data: buckets[key],
  }));
});
const areaYMax = computed(() =>
  Math.max(...areaSeriesData.value.flatMap((s) => s.data), 0),
);
const areaTicks = computed(() => niceTicks(areaYMax.value));
const areaTickTop = computed(
  () => areaTicks.value[areaTicks.value.length - 1] || 1,
);
const areaTickPos = computed(() =>
  areaTicks.value.map((v) => ({
    v,
    y: Math.round(175 - (v / areaTickTop.value) * 165),
  })),
);
const areaSeries = computed(() =>
  areaSeriesData.value.map((s) => {
    ...
    const areaPath =
      `M35,175 ` + pts.map((p) => `L${p.x},${p.y}`).join(" ") + ` L490,175 Z`;
    const linePath =
      `M${pts[0].x},${pts[0].y} ` +
      pts
        .slice(1)
...
<template>
  <div v-if="showSkeleton" class="overflow-auto">
    <div
    ...
    >
      <div
        class="w-full rounded-xl bg-white p-4 shadow-sm sm:p-5 lg:w-64 animate-pulse"
      >
        ...
        <div class="space-y-2">
          ...
          <div class="h-3 w-10/12 rounded bg-gray-100"></div>
        </div>
      ...
      <div
        class="grid w-full grid-cols-1 gap-4 md:min-h-0 md:flex-1 md:grid-cols-2 md:grid-rows-2 md:overflow-hidden"
      >
        <div
          v-for="n in 4"
          :key="`analytics-skeleton-${n}`"
          class="flex min-h-[260px] flex-col rounded-xl bg-white p-4 shadow-sm md:h-full md:min-h-0 animate-pulse"
        >
          <div class="mb-3 flex items-center justify-between">
            <div class="h-4 w-28 rounded bg-gray-200"></div>
            <div class="h-4 w-4 rounded-full bg-gray-200"></div>
          </div>
          <div class="flex-1 rounded-lg bg-gray-100"></div>
        </div>
      </div>
    </div>
  </div>
    ...
    >
      ...
      >
        <div class="flex-shrink-0">
          ...
          <p class="text-xs text-gray-600 leading-relaxed">
            The Accomplishment Report is a track record that contains all tasks
            you performed. This is the proof of your activity within the
            organisation.
          </p>
        </div>
        ...
        <div class="flex-1 overflow-y-auto lg:overflow-hidden">
          ...
          <ul
            class="text-xs text-gray-600 space-y-2 list-disc list-outside pl-4"
          >
            <li>
              Approved = approved by Director, Submitted = waiting Director
              approval, Pending = not yet submitted, Revision = returned for
              changes.
            </li>
            <li>
              Individual Report shows only your own approved tasks. Directors
              should use Unit Report to review Submitted, Pending, and Revision
              items across units.
            </li>
      ...
      >
        ...
        >
          ...
          <div
            class="flex min-h-0 flex-1 flex-col items-center justify-center gap-4 sm:flex-row sm:gap-6"
          >
            <svg
              viewBox="0 0 200 200"
        ...
        >
          ...
          <div class="min-h-0 flex-1 overflow-hidden">
            <svg viewBox="0 0 500 215" class="w-full h-full">
              ...
              <!-- Grid lines (dynamic) -->
              <line
                v-for="t in lineTickPos.slice(1)"
                :key="'lg' + t.v"
                x1="35"
                :y1="t.y"
                x2="495"
        ...
        >
          ...
          <div class="min-h-0 flex-1 overflow-hidden">
            <svg viewBox="0 0 500 200" class="w-full h-full">
              ...
              <!-- Grid lines (dynamic) -->
              <line
                v-for="t in areaTickPos.slice(1)"
                :key="'ag' + t.v"
                x1="35"
                :y1="t.y"
                x2="495"
          ...
          <!-- Legend -->
          <div
            class="mt-1 flex flex-wrap justify-center gap-3 pb-1 text-xs text-gray-700 sm:gap-4"
          >
            <div
              v-for="s in areaSeriesData"
              :key="s.key"

...
const hierarchyItems = computed(() => {
    ...
    if (parentTask.value) {
        items.push({
            label: parentTask.value.name,
            to: `/projects/${parentTask.value.parentId}/tasks`,
            title: parentTask.value.name,
        });
    } else {
        items.push({ label: "Task" });
    }

    items.push({ label: "Subtasks", current: true });
    return items;
});
...
const filtered = computed(() => {
    ...
    // No Subtasks/Insertions toggle: show all subtasks (non-design) for this parent task
...
const deleteTasks = async () => {
    ...
    try {
        ...
        await subtaskStore.deleteSubTasks(ids, parentId); // store handles auth filtering internally too
        selectedIds.value = new Set();
...
<template>
    <div id="page" class="flex flex-col h-full min-h-0">
        <div class="flex flex-col h-full min-h-0">
            ...
            <ProjectNavBar
                ...
                :placeholder-text="'Search Subtasks...'"
                @add="addTask = true"
            ...
            >
                ...
                <div class="flex-1 overflow-auto min-h-0">
                    <!--- Trying to emulate Svelte's {@const} -->
                    <template
                        v-for="preload in [{ preload: !isAlive || loading }]"
                        :key="preload"
                    >
                        <Loading
                            v-if="preload.preload"
                            :message="'Loading subtasks from the source...'"
                        />
                        <GridSubtasks
                            v-else-if="state === 'Grid View' && isAlive"
                            :subtasks="filtered"
                        ...
                        />
                        <TableSubtasks
                            v-else-if="state === 'Table View' && isAlive"
                            :subtasks="filtered"
                            :selectable="selectionMode"
                            :selected-ids="selectedIds"
                            :is-deletable="isDeletable"
                        ...
                        />
                        <ChartSubtasks
                            v-else-if="state === 'Chart View' && isAlive"
                            :subtasks="filtered"
                        />
                    </template>
                </div>

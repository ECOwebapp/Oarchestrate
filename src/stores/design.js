...
export const useDesignStore = defineStore("design", () => {
  ...
  const getPlenaryMembers = async () => {
    try {
      const response = await apiFetch("/design/plenary_members", {
        method: "GET",
      });
  ...
  const getDesignApprovalStatus = async (subtaskId) => {
    try {
      const response = await apiFetch(
        `/design/design_approval_status?subtaskId=${subtaskId}`,
        { method: "GET" },
      );
      const result = await response.json();
      if (response.ok) return result.data;
  ...
  const checkAllEngineersApproved = async (subtaskId) => {
    try {
      const response = await apiFetch(
        `/design/check_all_engineers_approval?subtaskId=${subtaskId}`,
        {
          method: "GET",
        },
      );
  ...
  const designReset = () => {
    plenary.value = designSubtasks.value = pduMembers.value = [];
  };

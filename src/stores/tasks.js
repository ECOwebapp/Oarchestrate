...
export const taskStore = defineStore("tasks", () => {
  ...
  const fetchTasks = async (parentId = null) => {
    ...
    try {
      ...
      const response = await apiFetch(`/tasks/fetch${query}`, {
        method: "GET",
      });
      ...
      for (const uhId of uhIds) {
        ...
        if (!existing) {
          await supabase.from("task_revision").insert({
            ...
            comment:
              message ||
              "From Unit Head: Output submitted — awaiting your review.",
          });
  ...
  const addTasks = async ({ mainTask }) => {
    ...
    if (mainTask.outputLink)
      await supabase
        .from("task_output")
        .upsert(
          { task_id: taskId, link: outputLink },
          { onConflict: "task_id" },
        );
  ...
  const fetchRevisions = async (taskId) => {
    ...
    const { data } = await supabase
      .from("task_revision")
      .select(
        "id, task_id, from_user, to_user, role, comment, is_read, created_at",
      )
      .eq("task_id", taskId)
